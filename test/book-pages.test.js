...
for (const language of bookPages) {
  test(`${language} book page presents the promotion and subscription context`, async () => {
    const html = await readFile(`src/books/magic-mushrooms-101/${language}/index.html`, "utf8");
    const promotion = html.match(/<aside class="book-promo"[\s\S]*?<\/aside>/)?.[0];
    ...
    assert.ok(getTextContent(promotion).length > 40, "promotion must include useful copy");

    assert.ok(subscriptionNote, "subscription form must have an introductory note");
    assert.ok(
      getTextContent(subscriptionNote).length > 20,
      "subscription note must contain visible text",
    );
    assert.match(html, /src="https:\/\/chillyhill\.substack\.com\/embed\?transparent=1"/);
  });
}
